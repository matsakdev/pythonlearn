...
def print_schedule_for_day(group_name, day):
...
def print_schedule_for_week(group_name, week):
    group_id = find_group_id(group_name)
...
def print_schedule_for_month(group_name, month):
    group_id = find_group_id(group_name)
    ...
    print('Розклад занять групи на місяць (', month_start.strftime('%B %Y'), ')')
    for event in group_events:
        event_start_time = timestamp_to_datetime(event['start_time'])
        event_end_time = timestamp_to_datetime(event['end_time'])
